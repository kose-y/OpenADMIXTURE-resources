...
function run_admixtures(n, p, k, prefixn, prefixp)
    ...
    sparsities = convert(Vector{Int}, [0.2, 0.175, 0.15, 0.125, 0.1, 0.075, 0.05, 0.025] .* p)
    for s in sparsities
        filename = "rslts/sim_path_n$(prefixn)_$(prefixp)_$(k)pops_path_$(s).bed"
        println(filename)
        d, _, _ = OpenADMIXTURE.run_admixture(filename, k; T=Float64, use_gpu=true)
        writedlm( "rslts/sim_n$(prefixn)_$(prefixp)_$(k)pops_path_$(s)_sp.Q.csv",  transpose(d.q), ',')
        #writedlm( "rslts/sim_n$(prefixn)_$(prefixp)_$(k)pops_path_$(s)_sp.P.csv",  transpose(d.p), ',')
    end
    filename = "n$(prefixn)_$(prefixp)_$(k)pops.bed"
    println(filename)
    d, _, _ = OpenADMIXTURE.run_admixture(filename, k; T=Float64, use_gpu=true)
    writedlm( "rslts/sim_n$(prefixn)_$(prefixp)_$(k)pops_full.Q.csv",  transpose(d.q), ',')
    #writedlm( "rslts/sim_n$(prefixn)_$(prefixp)_$(k)pops_full.P.csv",  transpose(d.p), ',')
end
